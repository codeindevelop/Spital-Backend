...
namespace Modules\Auth\App\Http\Controllers\user\VerifyMobile;
...
class UserVerifyMobileController extends Controller
{
    public function setUserMobileNumber(Request $request)
    {
        $user = Auth::user();
...
//                ->log('send new otp code for verify mobile');
    ...
    public function verifyMobileOTP(Request $request): JsonResponse
    {
        ...
        $token = UserVerify::where('mobile_verify_token', $request->mobile_verify_token)->first();
...
//                ->log('user verify by mobile');
    ...
    // Get OTP Code Again
    public function getMobileConfirmCodeAgain(Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
